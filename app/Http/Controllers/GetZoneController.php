...
class GetZoneController extends Controller
{
    ...
    public function getMultiCityArea(Request $request)
    {
        $cityIds = $request->city_id;
    
        if ($cityIds === "all" || (is_array($cityIds) && in_array("all", $cityIds))) {
            $zones = DB::table('ev_tbl_area')->select('id','Area_name')->whereIn('city_id',$cityIds)->where('status', 1)->get();
            return response()->json([
                'success' => true,
                'data' => $zones
            ]);
        ...
        $zones = DB::table('ev_tbl_area')->select('id','Area_name')->whereIn('city_id', $cityIds)
                      ->where('status', 1)
                      ->get();
