...
/**
     * Get the logged-in B2B user and guard.
     *
     * @return array|null
     *     [
...
/**
 * Check permission for B2B users (Master / Zone) based on active guard.
 *
 * - If guard = master → uses "B2B Master" role permissions
 * - If guard = zone   → uses "B2B Zone" role permissions
 *
 * @param  string  $permission   The permission name to check
 * @return bool                  True if the role tied to guard has the permission
 */
function b2bCan(string $permission): bool
{
    $auth = b2bAuth(); // ['user' => ..., 'guard' => ...]
    if (!$auth) {
        return false;
    }
    ...
    $roleName = $map[$guard] ?? null;
    if (!$roleName) {
        return false;
    }

    return \Spatie\Permission\Models\Role::where('id', $roleName)
        ->first()
