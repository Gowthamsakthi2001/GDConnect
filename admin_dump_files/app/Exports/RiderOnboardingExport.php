...
class RiderOnboardingExport implements FromCollection, WithHeadings, WithMapping
{
    protected $status;
    protected $from_date;
    protected $to_date;
    ...
    {
        $query = RiderOnboardingList::with(['deliveryman', 'customer', 'createdBy.get_role' ,'location_relation']);
        ...
        foreach ($this->selectedFields as $key) {
            switch ($key) {
                ...
                case 'client_id':
                    ...
                    break;
                case 'client_name':
                    $mapped[] = $row->customer->trade_name ?? '-';
                    break;
                case 'city':
    ...
    public function headings(): array
{
    ...
    foreach ($this->selectedFields as $field) {
        $fieldName = is_array($field) && isset($field['name']) ? $field['name'] : $field;
            switch ($fieldName) {
            case 'client_name':
                $headers[] = 'Client Name (Trade Name)';
                break;
            default:
