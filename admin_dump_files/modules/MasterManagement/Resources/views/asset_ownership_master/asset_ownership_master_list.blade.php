...
<style>
    ...
    <div class="main-content">
                ...
                <div class="card-header" style="background:#fbfbfb;">
                    <div class="row g-3">
                        <div class="col-md-6 d-flex align-items-center">
                            <div class="card-title h5 custom-dark m-0">Asset Ownership Master <span class="badge text-muted shadow ms-2 p-2" style="background:rgb(234,234,234);">{{$data->count()}}</span></div>
                        ...
                        <div class="col-md-6 d-flex gap-2 align-items-center justify-content-end">
                            ...
                            <div class="text-center d-flex gap-2">
                                ...
                                <div class="m-2 btn btn-success d-flex align-items-center px-3"
                                    style="cursor: pointer;"      data-bs-toggle="modal" data-bs-target="#AddAssetOwnershipeMaster">
        ...
        <div class="table-responsive">
                    <table id="AssetOwnershipTable_List" class="table text-left" style="width: 100%;">
                          <thead class="bg-white rounded" style="background:white !important; color:black !important;">
                            <tr>
                              ...
                              <th scope="col" class="custom-dark">Asset Ownership Name</th>
                        ...
                        <tbody class="bg-white border border-white">
                         @if(isset($data))
                            @foreach($data as $val)
                                ...
                                {{-- Action --}}
                                <td class="text-start">
                                    <a href="javascript:void(0);" class="text-success  editAssetOwnershipBtn" data-id="{{ $val->id }}"  style="font-size: 1.2rem;">
        ...
        <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasRightHR01" aria-labelledby="offcanvasRightHR01Label">
          <div class="offcanvas-header">
            <h5 class="custom-dark" id="offcanvasRightHR01Label">Asset Ownership Master</h5>
            <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
          </div>
          <div class="offcanvas-body">
        
            <div class="d-flex gap-2 mb-3">
                <button class="btn btn-outline-secondary w-50" onclick="clearAssetOwnershipFilter()">Clear All</button>
                <button class="btn btn-success w-50" onclick="applyAssetOwnershipFilter()">Apply</button>
            </div>
        ...
        <div class="modal fade" id="AddAssetOwnershipeMaster" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
            aria-labelledby="AddAssetOwnershipeMaster" aria-hidden="true">
            <form id="AddAssetOwnershipeMaster_From" action="javascript:void(0);" method="POST">
                @csrf
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" >Create Asset Ownership</h5>
                        ...
                        <div class="modal-body">
                            <div class="row">
                                 ...
                                 <div class="col-12 mb-3">
                                    <label for="vehicle_type" class="mb-2 ms-1">Asset Ownership
                                    <span class="text-danger">*</span></label>
                                        
                                    <input class="form-control basic-single" placeholder="Enter Asset Ownership" name="asset_ownership_name" required>
                                </div>
            ...
            <div class="modal fade" id="EditAssetOwnershipMaster" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
            aria-labelledby="EditAssetOwnershipMaster" aria-hidden="true">
            <form id="EditAssetOwnershipMaster_From" action="javascript:void(0);" method="POST">
                @csrf
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" >Update Asset Ownership</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="row">
                                 ...
                                 <div class="col-12 mb-3">
                                    ...
                                    <input class="form-control basic-single" placeholder="Enter Asset Ownership" name="asset_owner_ship_name" id="asset_owner_ship_name" required>
                                </div>
...
<script>
        ...
        function applyAssetOwnershipFilter() {
    ...
    function clearAssetOwnershipFilter() {
        const url = new URL(window.location.href);
...
<script>
            ...
            $(document).ready(function () {
       $('#AssetOwnershipTable_List').DataTable({
            // dom: 'Blfrtip',
            // dom: 'frtip',
...
<script>
    ...
    $("#AddAssetOwnershipeMaster_From").submit(function (e) {
        e.preventDefault();
        ...
        var redirect = "{{ route('admin.Green-Drive-Ev.master_management.asset_ownership_master.list') }}";
        ...
        $.ajax({
            url: "{{ route('admin.Green-Drive-Ev.master_management.asset_ownership_master.store') }}",
            type: "POST",
            data: formData,
            contentType: false,
            processData: false,
            success: function (response) {
                if (response.success) {
                    ...
                    $("#AddAssetOwnershipeMaster").modal('hide');
                    form.reset();
                    setTimeout(function () {
      ...
      $(document).on('click', '.editAssetOwnershipBtn', function () {
        var id = $(this).data('id');

        $.ajax({
            url: "{{ route('admin.Green-Drive-Ev.master_management.asset_ownership_master.get_data', ':id') }}".replace(':id', id),
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}"
            },
            success: function (response) {
                if (response.success) {
                    ...
                    $('#asset_owner_ship_name').val(response.data.name);
                     $('#edit_status').val(String(response.data.status)).trigger('change'); // 🔥 fixed here


                    $('#EditAssetOwnershipMaster').modal('show');
                } else {
        ...
        $("#EditAssetOwnershipMaster_From").submit(function (e) {
        e.preventDefault();
        ...
        $.ajax({
            url: "{{ route('admin.Green-Drive-Ev.master_management.asset_ownership_master.update') }}", // Update route
            type: "POST",
            data: formData,
            contentType: false,
            processData: false,
            success: function (response) {
                ...
                if (response.success) {
                    toastr.success(response.message);
                    $('#EditAssetOwnershipMaster').modal('hide');
                    form.reset();
                    setTimeout(function () {
    ...
    $('.toggle-status').change(function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                // Proceed with AJAX
                $.ajax({
                    url: "{{ route('admin.Green-Drive-Ev.master_management.asset_ownership_master.status_update') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        id: Id,
...
<script>
  document.getElementById('exportBtn').addEventListener('click', function () {
    ...
    if (selected.length > 0) {
     ...
     const url = `{{ route('admin.Green-Drive-Ev.master_management.asset_ownership_master.export_asset_ownership_master') }}?${params.toString()}`;
