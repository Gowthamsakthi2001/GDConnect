  ...
  overflow-x: hidden !important;
    ...
    <div class="main-content">
                ...
                <div class="card-header" style="background:#fbfbfb;">
                    <div class="row g-3">
                        <div class="col-md-6 d-flex align-items-center">
                            <div class="card-title h5 custom-dark m-0">Business Constitution Types<span class="badge text-muted shadow ms-2 p-2" style="background:rgb(234,234,234);">{{count($lists)}}</span>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6 d-flex gap-2 align-items-center justify-content-end">
                            ...
                            <div class="text-center d-flex gap-2">
                                ...
                                <div class="m-2 bg-white p-2 px-3 border-gray" onclick="BCT_Excel_Export()"><i class="bi bi-download fs-17 me-1"></i> Export</div>
                                ...
                                <div class="m-2 btn btn-success d-flex align-items-center px-3"
                                    onclick="AddorEditConstitutionModal('0',this)"
        ...
        <div class="table-responsive">
                    <table id="ConstitutionTableList" class="table text-center" style="width: 100%;">
                                ...
                                <tr>
                                   ...
                                   <td>
                                       <a href="javascript:void(0);" 
                                           ...
                                           onclick="AddorEditConstitutionModal('{{ $val->id }}', this)" 
    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasRightConstitution01" aria-labelledby="offcanvasRightConstitution01Label">
          <div class="offcanvas-header">
            <h5 class="custom-dark" id="offcanvasRightConstitution01Label">Business Constitution Filter</h5>
            <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
          </div>
          <div class="offcanvas-body">
        
            <div class="d-flex gap-2 mb-3">
                <button class="btn btn-outline-secondary w-50" onclick="clearConstitutionFilter()">Clear All</button>
                <button class="btn btn-success w-50" onclick="applyConstitutionFilter()">Apply</button>
            </div>
     ...
     <div class="modal fade" id="AddorEditConstitution" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
        aria-labelledby="AddorEditConstitutionLabel" aria-hidden="true">
        <form id="AddorEdit_BCT_Form" action="javascript:void(0);" method="POST">
            @csrf
            <input type="hidden" name="edit_id" id="Edit_HYP_ID" value="">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="AddorEditConstitutionLabel">Enter a Details</h5>
                    ...
                    <div class="modal-body">
                        <div class="row">
                             ...
                             <div class="col-12 mb-3">
                                <label class="input-label mb-2 ms-1" for="name">Name <span
                                        class="text-danger">*</span></label>
                                <input type="text" name="constitution_name" id="constitution_name" class="form-control" placeholder="Enter a Name">
...
<script>
    ...
    function BCT_Excel_Export(){
        ...
        const url = `{{ route('admin.Green-Drive-Ev.master_management.hypothecation.export_hypthecation') }}?${params.toString()}`;
        window.location.href = url;
    }
    
    $(document).ready(function () {
      $('#ConstitutionTableList').DataTable({
            // dom: 'Blfrtip',
            // dom: 'frtip',
    ...
    function applyConstitutionFilter() {
    ...
    function clearConstitutionFilter() {
        const url = new URL(window.location.href);
  ...
  function AddorEditConstitutionModal(id, el) {
        if (id == 0) {
            $("#AddorEditConstitutionLabel").text("Enter a Details");
            $("#AddorEditConstitution").modal('show');
            $('#AddorEditConstitution form').trigger('reset');
        } else {
            $("#AddorEditConstitutionLabel").text("Update a Details");
            $(".submitBtn").text('Update');
    
            var hyp_name = $(el).data('name');
            var status = $(el).data('status');
    
            $("#AddorEditConstitution").modal('show');
            $("#Edit_HYP_ID").val(id);
            $('#constitution_name').val(hyp_name);
            $("#status").val(status);
        }
    }

  
  $("#AddorEdit_BCT_Form").submit(function(){
        var form = $(this)[0];
        var formData = new FormData(form);
        formData.append("_token","{{csrf_token()}}");
        var redirect = "{{ route('admin.Green-Drive-Ev.master_management.hypothecation.index') }}";
        $.ajax({
            url: "{{ route('admin.Green-Drive-Ev.master_management.hypothecation.store') }}",
            type: "POST",
            data:formData,
            ...
            success: function(response) {
                ...
                if (response.success == true) {
                   toastr.success(response.message);
                    $("#AddorEditConstitution").modal('hide');
                    form.reset();
                   setTimeout(function(){
...
<script>
    ...
    function RightSideFilerOpen(){
        const bsOffcanvas = new bootstrap.Offcanvas('#offcanvasRightConstitution01');
        bsOffcanvas.show();
    }
...
<script>
    ...
    $('.toggle-status').change(function (e) {
        ...
        Swal.fire({
            title: 'Are you sure?',
            text: `You are about to ${intendedStatus ? 'Active' : 'Inactive'} this Constitution Type.`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: "{{ route('admin.Green-Drive-Ev.master_management.hypothecation.status_update') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        id: brandId,
...
function DeleteRecord(id){
    Swal.fire({
            title: 'Are you sure?',
            text: `You want to delete this Constitution Type?`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
