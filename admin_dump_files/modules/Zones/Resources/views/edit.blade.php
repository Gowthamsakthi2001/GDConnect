...
<style>
    ...
    <div class="main-content">
                ...
                <div class="row g-3">
                    <div class="col-md-4 d-flex align-items-center">
                        <div class="card-title h5 custom-dark m-0"> <a
                                ...
                                class="btn btn-sm shadow me-2"><i class="bi bi-arrow-left"></i> </a>Update a Zone
                        </div>
                    </div>
                </div>
        ...
        <div class="card my-3">
            ...
            <div class="card-body pt-3">
                <form action="javascript:void(0);" method="POST" id="EditzoneForm">
                    @csrf
                    <input type="hidden" id="ZoneEditUrl" value="{{ route('admin.Green-Drive-Ev.zone.update', ['id' => $zone->id]) }}">
                    <div class="row">
                        <!-- Left side -->
                        <div class="col-md-4">
                                ...
                                <select class="form-select custom-select2-field form-control-sm" 
                                    ...
                                    @if(isset($states))
                                        @foreach($states as $val)
                                            <option value="{{$val->id}}" {{$val->id == $zone->state_id ? 'selected' : ''}}>{{$val->state_name}}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                                    ...
                                    <option value="">Select a state first</option>
                                     @if(isset($cities))
                                        @foreach($cities as $val)
                                            <option value="{{$val->id}}" {{$val->id == $zone->city_id ? 'selected' : ''}}>{{$val->city_name}}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" name="zone_name" id="zone_name"
                                       placeholder="" value="{{$zone->name ?? ''}}">
                                <small id="edit_zone_name_feedback" class="text-danger"></small>
                            </div>


                            <div class="mb-3">
                                <label class="input-label mb-2 ms-1">Latitude <span class="text-danger fw-bold">*</span></label>
                                <input type="number" class="form-control" name="latitude" id="lat" value="{{$zone->lat ?? ''}}" readonly>
                            </div>

                            <div class="mb-3">
                                <label class="input-label mb-2 ms-1">Longitude <span class="text-danger fw-bold">*</span></label>
                                <input type="number" class="form-control" name="longitude" id="long" value="{{$zone->long ?? ''}}" readonly>
                            </div>
                        </div>
                        ...
                        <div class="col-md-8">
                            <div class="mb-3">
                                ...
                                <input id="searchInput" name="search_address" class="form-control mb-2" type="text"
                                    placeholder="Search for a location" value="{{$zone->address ?? ''}}">
                    ...
                    <div class="row mt-3">
                        <div class="col-md-12 text-end">
                            <a href="{{ route('admin.Green-Drive-Ev.zone.list') }}" class="btn btn-secondary me-2">Cancel</a>
                            <button type="submit" class="btn btn-success" id="ZoneCreateBtn">Update Zone</button>
                        </div>
                    </div>
    ...
    <script>
        ...
        let LatiTude = {{ $zone->lat ?? 13.010456533174729 }};
        let LongiTude = {{ $zone->long ?? 80.2707 }};
        
        function initMap() {
            const centerLatLng = { lat: LatiTude, lng: LongiTude };
            ...
            google.maps.event.addListener(searchBox, 'place_changed', function() {
                ...
                if (place.geometry) {
                    ...
                    if (clickMarker) clickMarker.setMap(null); // remove previous marker
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.success) {
                        ...
                        $('#edit_zone_name_feedback').text('').removeClass('text-danger') .removeClass('text-success');
                        Swal.fire({
                            icon: 'success',
                            title: 'Updated!',
