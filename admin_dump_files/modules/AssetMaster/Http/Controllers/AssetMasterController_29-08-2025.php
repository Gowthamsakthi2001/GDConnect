...
class AssetMasterController extends Controller
{
      ...
      public function asset_manage_dashboard(Request $request)
    {
        ...
        $vehicle_models = DB::table('ev_tbl_vehicle_models')->where('status',1)->select('id','vehicle_model')->get();
        $vehicle_types = VehicleType::where('is_active', 1)->select('id','name')->get();
        // $customers = CustomerMaster::where('status', 1)->select('id', 'trade_name')->limit(50)->get();                                      

        
        $map_api_key = BusinessSetting::where('key_name', 'google_map_api_key')->value('value');

        $inventory_locations = InventoryLocationMaster::where('status',1)->select('id','name')->get();
        $location_data = LocationMaster::where('status', 1)->select('id','name')->get();
    
        $assetWiseTable = DB::table('vehicle_qc_check_lists as qc')
            ->select(
                'bm.brand_name',
                'vm.vehicle_model',
                'vt.name as vehicle_type_name',   // ✅ vehicle type name
                'qc.vehicle_type',
            ...
            ->leftJoin('vehicle_types as vt', 'qc.vehicle_type', '=', 'vt.id')  // ✅ join vehicle_types
        ...
        $total_assets    = $assetWiseTable->sum('vehicle_count');
        $reg_assets      = $assetWiseTable->sum('registered_vehicles');
        $unreg_assets    = $total_assets - $reg_assets;

        $assetReg_percentage   = $total_assets > 0 ? round(($reg_assets / $total_assets) * 100, 2) : 0;
        $assetUnreg_percentage = $total_assets > 0 ? round(($unreg_assets / $total_assets) * 100, 2) : 0;
        ...
        // $qcWiseTable = DB::table('vehicle_qc_check_lists as qc') //qc status
        //     ->leftJoin('ev_tbl_location_master as loc', 'qc.location', '=', 'loc.id')
        //     ->where('qc.delete_status', 0)
        //     ->when($location_id != "", function ($query) use ($location_id) {
        //         return $query->where('qc.location', $location_id);
        //     })
        ...
        //         return $query->where('qc.vehicle_type', $vehicle_type);
        //     })
        //     ->groupBy('qc.location', 'loc.name')
        //     ->selectRaw("
        //         qc.location as location_id,
        //         loc.name as location_name,
        //         COUNT(qc.id) as qc_total_count,
        //         SUM(CASE WHEN qc.status = 'pass' THEN 1 ELSE 0 END) as qc_pass_count,
        //         SUM(CASE WHEN qc.status = 'fail' THEN 1 ELSE 0 END) as qc_fail_count,
        //         SUM(CASE WHEN qc.status IS NULL OR qc.status NOT IN ('pass','fail') THEN 1 ELSE 0 END) as qc_pending_count
        //     ")
        //     ->get();
        
        // $qc_total_count = $qcWiseTable->sum('qc_total_count');
        // $qc_pass_count = $qcWiseTable->sum('qc_pass_count');
        // $qc_fail_count = $qcWiseTable->sum('qc_fail_count');
        // $qc_pending_count = $qcWiseTable->sum('qc_pending_count');

        // $qc_pass_percentage    = $qc_total_count > 0 ? round(($qc_pass_count / $qc_total_count) * 100, 2) : 0;
        // $qc_fail_percentage    = $qc_total_count > 0 ? round(($qc_fail_count / $qc_total_count) * 100, 2) : 0;
        // $qc_pending_percentage = $qc_total_count > 0 ? round(($qc_pending_count / $qc_total_count) * 100, 2) : 0;


        $clientWiseTable = DB::table('vehicle_qc_check_lists as qc') //map sql query
        ->leftJoin('ev_tbl_asset_master_vehicles as vh', 'qc.id', '=', 'vh.qc_id')
        ->leftJoin('asset_vehicle_inventories as inv', 'inv.asset_vehicle_id', '=', 'vh.id')
        ->leftJoin('ev_tbl_location_master as lo', 'lo.id', '=', 'qc.location')
        ->where('qc.delete_status', 0)
        ...
        $v_models = DB::table('vehicle_qc_check_lists as qc')
        ->leftJoin('ev_tbl_vehicle_models as vm', 'qc.vehicle_model', '=', 'vm.id')
        ->select(
            'qc.vehicle_model',
            'vm.vehicle_model as model_name',
            DB::raw('COUNT(qc.vehicle_model) as model_count')
        )
        ->when($vehicle_model != "", function ($query) use ($vehicle_model) {
            return $query->where('qc.vehicle_model', $vehicle_model);
        })
        ->groupBy('qc.vehicle_model', 'vm.vehicle_model')
        ->get();
        ...
        $inventory_summary = AssetVehicleInventory::with([ //data table
            'assetVehicle.vehicle_model_relation',
            'assetVehicle.vehicle_type_relation',
            'inventory_location'
        ])->whereNotNull('asset_vehicle_id')->orderBy('id', 'desc')->limit(30)->get();
        

        
        return view('assetmaster::asset_management_dashboard', compact( //updated by Mugesh B
            'timeline', 'from_date', 'to_date','vehicle_types','vehicle_models','vehicle_type','vehicle_model','v_models','brandWiseData',
            //updated By Gowtham S
            'map_api_key','location_id','MapchartData','inventory_locations','total_asset_count','onRoad_asset_count','offRoad_asset_count','onRoad_percentage','offRoad_percentage','accident_asset_count','accidentcase_percentage','inventory_summary','reg_assets','unreg_assets','assetReg_percentage','assetUnreg_percentage','clientWiseTable','location_data','city_table_data','document_alerts','document_validity_count'
        ));
...
public function vehicle_bulk_upload_form_import(Request $request)
{
    ...
    if (!empty($data[$field])) {
        try {
            ...
            // 🔹 Force format check before parsing
