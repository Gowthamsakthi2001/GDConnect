...
class CityDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($query) {
                return $query->actionBtn('city-table');
            })
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('short_code')->title('Short Code')->defaultContent('N/A'),
    ...
    private function statusBtn($City): string
    {
        $isChecked = $City->status ? 'checked' : '';
    
        $status = '<div class="form-check form-switch">';
        $status .= '<label class="toggle-switch" for="statusCheckbox_' . $City->id . '">';
        $status .= '<input type="checkbox" onclick="status_change_alert(\'' . route('admin.Green-Drive-Ev.City.status', [$City->id, $City->status ? 0 : 1]) . '\', \'' . ($City->status ? 'Deactivate' : 'Activate') . ' this City?\', event)" class="form-check-input toggle-btn" id="statusCheckbox_' . $City->id . '" ' . $isChecked . '>';
