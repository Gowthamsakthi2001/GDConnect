...
<script>
  ...
  $(document).ready(function () {
    ...
    serverSide: true,
    ajax: {
        url: "{{ route('b2b.admin.accident_report.list') }}",
        type: "GET",
        data: function(d) {
    ...
    columns: [
            ...
            { data: 15 },
            { data: 16 },
            { data: 17, orderable: false, searchable: false } // Action
            ],
    order:[[1,'desc']],
...
// });
  ...
  document.getElementById('export_download').addEventListener('click', function () {
    ...
    // const url = `{{ route('b2b.admin.accident_report.export') }}?${params.toString()}`;
    ...
    $.ajax({
        url: "{{ route('b2b.admin.accident_report.export') }}",
        method: "GET",
        data: data,
        ...
        success: function(blob) {
            ...
            link.download = "accident_request_list-" + new Date().toISOString().split('T')[0] + ".csv";
...
// When modal opens, set the accident ID
$(document).on('click', '.accident-status-btn', function() {
    accidentId = $(this).data('id');
    assignId = $(this).data('assign_id');
});
...
$('#accidentStatusModal').on('hidden.bs.modal', function () {
    $('#accidentStatusForm')[0].reset();            // reset inputs
    $('#accidentStatusForm').find('select').val('').trigger('change'); // reset select2
    $('#accidentStatusForm button[type="submit"]').prop('disabled', false).html(
        'Update Accident Status'
    );
});

$('#accidentStatusForm').on('submit', function(e) {
    e.preventDefault();
    ...
    $.ajax({
        url: "{{ route('b2b.admin.accident_report.updateStatus') }}",
        type: "POST",
        data: {
            _token: "{{ csrf_token() }}",
            id: accidentId,
        ...
        success: function(response) {
            if(response.success){
                ...
                $('#accidentStatusModal').modal('hide');
                accidentTable.ajax.reload(null, false); // reload table without resetting pagination
            } else {
                toastr.error(response.message || "Something went wrong");
                // reset button if failed
                $submitBtn.prop('disabled', false).html('Update Accident Status');
            }
        },
        error: function() {
