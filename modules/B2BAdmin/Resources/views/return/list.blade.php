...
@endsection
    <div class="main-content">
     ...
     <!--               </div>-->
                    <table id="returnList" class="table text-left table-striped table-bordered table-hover" style="width: 100%;">
...
<script>
     ...
     $(document).ready(function () {
        // $('#loadingOverlay').show();
         ...
         table = $('#returnList').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('b2b.admin.return_request.list') }}",
                type: 'GET',
                 data: function (d) {
  ...
  document.getElementById('export_download').addEventListener('click', function () {
    ...
    // const url = `{{ route('b2b.admin.return_request.export') }}?${params.toString()}`;
    ...
    $.ajax({
        url: "{{ route('b2b.admin.return_request.export') }}",
        method: "GET",
        data: data,
        ...
        success: function(blob) {
            ...
            link.download = "return_request_list-" + new Date().toISOString().split('T')[0] + ".csv";
