            ...
            <div class="card">
               <div class="card-header" style="background:#eef2ff;">
                ...
                <div class="card-body">
                   ...
                   <form id="Re_ApproveAssetMasterVehicleForm" action="javascript:void(0);" method="post" enctype="multipart/form-data">
                        @csrf
                      <div class="row">
                     ...
                     <div class="col-md-6 mb-3">
                      ...
                      <div class="col-md-6 mb-3">
                        ...
                        <div class="col-12 text-end gap-4 d-block" id="status_update_btn_section">
                            <button type="button" class="btn btn-danger px-5"  id="rejectBtn"
                                onclick="ReUpdate_VehicleAcceptOrRejectStatus('{{$vehicle_data->id}}', 'Do you want to update the Asset Master Vehicle data and Reject status?', 'rejected')">
                                Reject
                            </button>
                        
                            <button type="submit" id="submitBtn" class="btn btn-success px-5"
                                onclick="ReUpdate_VehicleAcceptOrRejectStatus('{{$vehicle_data->id}}', 'Do you want to update the Asset Master Vehicle data and Accept status?', 'accepted')">
                                Approve
                            </button>
                        </div>
...
<script>
    ...
    // $("#Re_ApproveAssetMasterVehicleForm").submit(function(e) {
    ...
    //     $.ajax({
    //         url: "{{ route('admin.asset_management.asset_master.reupdate_vehicle_data') }}",
    //         type: "POST",
    //         data: formData,
    //         processData: false,
    //         contentType: false,
...
function ReUpdate_VehicleAcceptOrRejectStatus(id, message, status, title = "Are you sure?") {
    var form = $("#Re_ApproveAssetMasterVehicleForm")[0];
    var formData = new FormData(form);


     var clickedBtn = status === 'accepted' ? $("#submitBtn") : $("#rejectBtn");
    var originalBtnText = clickedBtn.html();

    clickedBtn.prop("disabled", true).html("⏳ Processing...");
        
    formData.append("id", id);
    formData.append("_token", "{{ csrf_token() }}");
    formData.append("status", status);

    var route = "{{ route('admin.asset_management.asset_master.reupdate_vehicle_data') }}";
    
    
    const resetClickedButton = () => {
        clickedBtn.prop("disabled", false).html(originalBtnText);
    };
    ...
    if (status === 'accepted') {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                formData.append("reject_remark", Remarks);
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success) {
                            ...
                            showToast('error', 'Warning! ' + response.message);
                            resetClickedButton();
                        }
                    },
                    error: function () {
