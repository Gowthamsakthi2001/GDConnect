                ...
                <div class="row g-3 d-flex justify-content-between align-items-center">
                    ...
                    <div class="col-md-6 col-12 mb-2">
                        <div class="d-flex justify-content-start align-items-center">
                            ...
                            <div class="px-3">
                                <div class="h4 fw-bold mt-2">Vehicle ID : {{$vehicle_data->id ?? '' }} Logs & Histories</div>
                            </div>
                        </div>
                    </div>
                    <?php
                    //   $log_id = encrypt($log_id);
                    //   $asset_vehicle_id = encrypt($asset_vehicle_id);
                    ?>
                    <div class="col-md-6 col-12 mb-2">
                        <div class="d-flex justify-content-end align-items-center gap-2">
                            <a href="{{route('admin.asset_management.asset_master.logs_history')}}" class="btn btn-dark edit-candidate-btn px-5"><i class="bi bi-arrow-left me-2"></i> Back</a>
                        </div>
                    </div>
                  ...
                  <div class="tab-pane fade show active" id="pills-assets-section" role="tabpanel" aria-labelledby="pills-assets-section-tab" tabindex="0">
                           <div class="card">
                                <div class="card-header" style="background:#edfcff;">
                                    <h5 style="color:#1b4d5e;" class="fw-bold">Asset Details</h5>
                                    <p class="mb-0" style="color:#1b4d5e;">View all Asset in Details</p>
                                </div>
                                ...
                                <select class="form-select custom-select2-field form-control-sm" name="leasing_partner" id="leasing_partner">
                                    ...
                                    @if(isset($leasing_partner))
                                       @foreach($leasing_partner as $val)
                                          <option value="{{$val->id}}" {{$vehicle_data->leasing_partner == $val->id ? 'selected' : ''}}>{{$val->name}}</option>
                                       @endforeach
                                    @endif
                                </select>
                            </div>
                     ...
                     <div class="col-md-6 mb-3">
                        ...
                        <div class="col-md-6 mb-3">
                            <div class="form-group">
                                ...
                                <!--<input type="text" class="form-control bg-white" name="battery_type" id="battery_type"  value="{{$vehicle_data->battery_type ?? ''}}" placeholder="Enter Battery Type" >-->
                                ...
                                <div class="card-body custom-card-body">
                                     <div class="row">
                                         <div class="col-12 my-4 text-center">
                                             <h5 class="fw-bold">Vehicle ID : {{$vehicle_data->id ?? '' }}</h5>
                                         </div>
                                        @if(isset($history_logs) && count($history_logs))
                                           @foreach($history_logs as $val)
                                        
                                              <div class="col-12 mb-5">
                                                    <div class="form-group">
                                                        <div class="d-flex justify-content-between">
                                                            <div class="mb-3">
                                                                <label class="input-label">{{$val->get_comment_user->name ?? ''}} ({{$val->get_comment_user->get_role->name ?? ''}})</label><br>
                                                                <small class="input-label">{{date('d M Y h:i:s A',strtotime($val->created_at)) ?? ''}}</small>
                                                            </div>
                                                            <div class="">
                                                                @if($val->status_type == "uploaded")
                                                                <button class="btn btn-primary px-5">Asset Uploaded</button>
                                                                @elseif($val->status_type == "accepted")
                                                                <button class="btn btn-success px-5">Asset Accepted</button>
                                                                @elseif($val->status_type == "rejected")
                                                                <button class="btn btn-danger px-5">Asset Rejected</button>
                                                                @endif
                                                            </div>
                                                            
                                                        </div>
                                                       <div class="form-group">
                                                            <textarea name="remarks" class="form-control">{{$val->remarks ?? ''}}</textarea>
                                                        </div>
                                                    </div>
                                                </div>
                                           @endforeach
