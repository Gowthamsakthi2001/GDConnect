...
$qc_array = QualityCheck::where('location',2)->pluck('chassis_number')->toArray(); // get DB numbers
$missing_numbers = array_diff($hyderbad_numbers, $qc_array); // numbers in your list but missing in DB
$exists_numbers = array_intersect($hyderbad_numbers, $qc_array);
$total_count = count($qc_array);
$missing_numbers_count = count($missing_numbers);
$exists_numbers_count = count($exists_numbers);

dd($total_count,array_values($missing_numbers),$missing_numbers_count,$exists_numbers_count);
    ...
    return view('assetmaster::quality_check.index', [
       ...
       public function add_quality_check(Request $request){
        ...
        return view('assetmaster::quality_check.create' , compact('vehicles' , 'vehicle_types' , 'location' , 'check_lists'));
    }
        ...
        public function get_qc_checklist(Request $request)
    {
     ...
     public function view_quality_check(Request $request , $id){
       ...
       ->whereIn('status', ['pass', 'fail'])
        ...
        return view('assetmaster::quality_check.view' , compact('datas' , 'vehicles' , 'initiate_values' , 'vehicle_types' , 'location'));
    }
