    ...
    <div class="container-fluid d-flex justify-content-between align-items-center">
        ...
        <ul class="navbar-nav d-flex align-items-center mb-0" >
            ...
            <li class="nav-item dropdown user_profile custom-dropdown">
                <a class=" d-flex align-items-center gap-3 bg-white p-2 m-1 " style="position:relative;max-height:58px"
                   href="#" role="button" id="profileDropdownButton" aria-expanded="false">
                    <div class="user_img border rounded-circle overflow-hidden" style="width: 40px; height: 40px;">
                        <img src="{{ $img }}" alt="User Image" class="img-fluid w-100 h-100 object-fit-cover">
                    </div>
                    <div class="text-start">
                        <small style="font-size:12px;" class="text-secondary text-muted">{{ ucfirst($user->type) }}</small>
            ...
            <div class="row align-items-center">
                ...
                <div class="col-auto">
                    <div class="d-flex gap-3" >
                        <a href="{{ route('b2b.dashboard') }}" class="btn btn-dark d-flex align-items-center gap-2" style="height:40px;max-width:84px">
...
async function loadVehicleCard(imei,vehicleNumber,vehicleType) {
    ...
    if (!response?.data?.vehicles || response.data.vehicles.length === 0) {
        // 🚨 No vehicle data found → fallback
        statusContainer.innerHTML = `
    ...
    // ✅ pick first vehicle (or loop if you need multiple)
...
async function populateRunningVehiclesDropdown() {
    ...
    try {
        ...
        const response = await fetch(`{{ route('b2b.mobitra_api.tracking_json') }}?page=1`);
        const data = await response.json();
...
async function updateVehiclesJson(imei) {
    ...
    let urlJson = `{{ route('b2b.mobitra_api.update_vehicles_json') }}?imei=${imei}`;
...
async function loadMoreVehicles(search = "") {
    ...
    try {
        const response = await fetch(
            `{{ route('b2b.mobitra_api.tracking_json') }}?page=${page}&search=${encodeURIComponent(search)}`
        );
        const data = await response.json();
