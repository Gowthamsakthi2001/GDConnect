@extends('layouts.b2b')
                        ...
                        <a class="text-decoration-none d-flex align-items-center" href="#" role="button" id="userDropdown" data-bs-toggle="dropdown" aria-expanded="false" style="max-height:56px;width:185px;">
                            <div class="d-flex align-items-center gap-2 bg-white p-1 rounded">
                                ...
                                <div class="text-start d-none d-md-block">
                                    ...
                                    <small class="text-secondary" style="font-size:12px">{{auth()->user()->email ?? '' }}</small>
                                </div>
    ...
    <div class="sidebar-content d-flex flex-column" >
        ...
        <div class="vehicle-list-container flex-grow-1" 
            ...
            @foreach(array_slice($data['vehicles'], 0, 50) as $vehicle)
   ...
   <script>
    ...
    async function fetchVehicleData(vehicleNumber) {
        try {
            ...
            `;
            const url = "{{ route('b2b.mobitra_api.get_user_devices') }}" + `?vehicle_number=${vehicleNumber}`;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        searchInput.addEventListener('input', function() {
            if (this.value.trim() === "" && document.querySelector(".filter-btn.active").getAttribute("data-filter") === "all") {
                // Reset back to unfiltered mode
                console.log("testing");
                isFiltered = false;
                offset = 0;
                allLoaded = false;
                // document.getElementById("vehicleListContainer").innerHTML = "";
...
async function updateVehiclesJson(){
    let urlJson="{{route('b2b.mobitra_api.update_vehicles_json')}}"
...
async function loadMoreVehicles() {
    ...
    try {
        ...
        const nextVehicles = allVehicles.slice(offset, offset + limit);
...
document.getElementById("vehicleListContainer").addEventListener("scroll", function () {
    ...
    scrollTimeout = setTimeout(() => {
        ...
        if (container.scrollTop + container.clientHeight >= container.scrollHeight - 5) {
            if (isFiltered) {
                renderFilteredVehicles(false); // load more filtered
            } else {
                loadMoreVehicles(); // existing function
            }
        }
    }, 200);
});
...
function handleCardSelection(card) {
    ...
    const vehicleStatus = card.getAttribute('data-vehicle-status');
...
function clearContainerKeepLoader() {
    ...
    if (loader && container.contains(loader)) {
        loader.remove(); // temporarily take it out
    }

    container.innerHTML = ""; // clear all cards

    if (loader) {
        container.appendChild(loader); // put loader back at the end
    }
}
