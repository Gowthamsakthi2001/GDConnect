...
class B2BTrackingController extends Controller
{
    public function mobitra_api_setting()
    {
        return view('b2b::mobitra_api.mobitra_api_settings');
    }
...
public function getNotifications(Request $request)
{
    ...
    try {
        ...
        $params = [
            ...
            'offset' => max(0, (int)$request->input('offset', 1)),
        ];
...
public function mobitra_tracking(Request $request)
{
    ...
    // Riders created by this user
    $riders = B2BVehicleRequests::where('is_active', 1)
    ->whereHas('rider.customerLogin', function ($q) use ($customerId) {
        // Only check customer_id from customerLogin
        $q->where('customer_id', $customerId);
    })
    ->when($guard === 'master', function ($q) use ($user) {
        // Apply city filter directly on B2BVehicleRequests
        $q->where('city_id', $user->city_id);
    })
    ->when($guard === 'zone', function ($q) use ($user) {
        // Apply city + zone filter directly on B2BVehicleRequests
        $q->where('city_id', $user->city_id)
    ...
    $vehicleIds = B2BVehicleAssignment::whereNotNull('asset_vehicle_id')
        ->where('status','running')
        ->whereIn('rider_id', $riders)
    ...
    return view('b2b::mobitra_api.tracking', compact('vehicles', 'api_mode'));
}
...
//   return view('b2b::mobitra_api.tracking',compact('results','api_mode'));
// }
