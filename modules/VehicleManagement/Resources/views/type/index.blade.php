  ...
  overflow-x: hidden !important;
                ...
                <div class="card-header" style="background:#fbfbfb;">
                    <div class="row g-3">
                        <div class="col-md-6 d-flex align-items-center">
                            <div class="card-title h5 custom-dark m-0">Vehicle Types <span class="badge text-muted shadow ms-2 p-2" style="background:rgb(234,234,234);">{{count($lists)}}</span></div>
                        ...
                        <div class="col-md-6 d-flex gap-2 align-items-center justify-content-end">
                            ...
                            <div class="text-center d-flex gap-2">
                                <div class="m-2 bg-white p-2 px-3 border-gray" onclick="window.location.href='{{route('admin.vehicle.type.export')}}'"><i class="bi bi-download fs-17 me-1"></i> Export</div>
                        ...
                        @if(isset($lists))
                            @foreach($lists as $val)
                                <?php
                                    ...
                                    $colorClass = match ($val->is_active) {
                                        1 => 'text-success',
                                        0 => 'text-danger',
                                        default => 'text-secondary',
                                    };
                                     
                                    $statusName = $val->is_active == 1 ? 'Active' : 'Inactive';
                                ?>
    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasRightHR01" aria-labelledby="offcanvasRightHR01Label">
          <div class="offcanvas-header">
            <h5 class="custom-dark" id="offcanvasRightHR01Label">Vehicle Types Filter</h5>
           ...
           <div class="card mb-3">
               ...
               <div class="card-body">
                    <div class="form-check mb-3">
                      <input class="form-check-input" {{$ch_status == 'all' ? 'checked': ''}} type="radio" name="status" id="status" value="all" >
     ...
     <div class="modal fade" id="AddorEditVT" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
        aria-labelledby="AddorEditVTLabel" aria-hidden="true">
        <form id="AddorEditVT_Form" action="javascript:void(0);" method="POST">
            @csrf
            <input type="hidden" name="type_id" id="EditTypeId" value="">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="AddorEditVTLabel">Enter a Details</h5>
                        ...
                        <div class="row">
                            <div class="col-12 mb-3">
                                ...
                                <input type="text" name="name" id="name" class="form-control"
                                    placeholder="{{ __('Enter a Type Name') }}" value="{{ old('name') }}"
                                    maxlength="191">
...
<script>
  ...
  function AddorEditVTModal(id,el) {
            ...
            if (id == 0) {
                 $("#AddorEditVT").modal('show');
                $('#AddorEditVT form').trigger('reset');
                
            } else {
                $("#AddorEditVTLabel").text("Update a Details");
  ...
  $("#AddorEditVT_Form").submit(function(){
        var form = $(this)[0];
        var formData = new FormData(form);
        formData.append("_token","{{csrf_token()}}");
        var redirect = "{{ route('admin.vehicle.type.index') }}";
        $.ajax({
            url: "{{ route('admin.vehicle.type.store') }}",
            type: "POST",
            data:formData,
    ...
    $('.toggle-status').change(function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                // Proceed with AJAX
                $.ajax({
                    url: "{{ route('admin.vehicle.type.status_update') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        id: brandId,
...
function DeleteRecord(id){
    Swal.fire({
            title: 'Are you sure?',
            text: `You Want to Delete this Vehicle Type?`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: "{{ route('admin.vehicle.type.destroy') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        id: id,
