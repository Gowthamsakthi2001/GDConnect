...
<style>
    ...
    <div class="main-content">
                ...
                <div class="card-header" style="background:#fbfbfb;">
                    <div class="row g-3">
                        <div class="col-md-6 d-flex align-items-center">
                            <div class="card-title h5 custom-dark m-0">Financing Type Master <span class="badge text-muted shadow ms-2 p-2" style="background:rgb(234,234,234);">{{$data->count()}}</span></div>
                        ...
                        <div class="col-md-6 d-flex gap-2 align-items-center justify-content-end">
                            ...
                            <div class="text-center d-flex gap-2">
                                ...
                                <div class="m-2 btn btn-success d-flex align-items-center px-3"
                                    style="cursor: pointer;"      data-bs-toggle="modal" data-bs-target="#AddFinancingTypeMaster">
        ...
        <div class="table-responsive">
                    <table id="FinancingTypeTable_List" class="table text-left" style="width: 100%;">
                          <thead class="bg-white rounded" style="background:white !important; color:black !important;">
                            <tr>
                              ...
                              <th scope="col" class="custom-dark">Financing Type Name</th>
                        ...
                        <tbody class="bg-white border border-white">
                         @if(isset($data))
                            @foreach($data as $val)
                                ...
                                {{-- Action --}}
                                <td class="text-start">
                                    <a href="javascript:void(0);" class="text-success editTelemetricOEMBtn" data-id="{{ $val->id }}"  style="font-size: 1.2rem;">
        ...
        <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasRightHR01" aria-labelledby="offcanvasRightHR01Label">
          <div class="offcanvas-header">
            <h5 class="custom-dark" id="offcanvasRightHR01Label">Financing Type Master</h5>
            <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
          </div>
          <div class="offcanvas-body">
        
            <div class="d-flex gap-2 mb-3">
                <button class="btn btn-outline-secondary w-50" onclick="clearFinancingTypeFilter()">Clear All</button>
                <button class="btn btn-success w-50" onclick="applyFinancingTypeFilter()">Apply</button>
            </div>
        ...
        <div class="modal fade" id="AddFinancingTypeMaster" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
            aria-labelledby="AddFinancingTypeMaster" aria-hidden="true">
            <form id="AddFinancingTypeMaster_From" action="javascript:void(0);" method="POST">
                @csrf
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" >Create Financing Type</h5>
                        ...
                        <div class="modal-body">
                            <div class="row">
                                 ...
                                 <div class="col-12 mb-3">
                                    <label for="vehicle_type" class="mb-2 ms-1">Financing Type
                                    <span class="text-danger">*</span></label>
                                        
                                    <input class="form-control basic-single" placeholder="Enter Financing Type" name="financing_type_name" required>
                                </div>
            ...
            <div class="modal fade" id="EditFinancingTypeMaster" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
            aria-labelledby="EditFinancingTypeMaster" aria-hidden="true">
            <form id="EditFinancingTypeMaster_From" action="javascript:void(0);" method="POST">
                @csrf
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" >Update Financig Type</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="row">
                                 ...
                                 <div class="col-12 mb-3">
                                    ...
                                    <input class="form-control basic-single" placeholder="Enter Financing Type" name="financing_type_name" id="financing_type_name" required>
                                </div>
...
<script>
        ...
        function applyFinancingTypeFilter() {
    ...
    function clearFinancingTypeFilter() {
        const url = new URL(window.location.href);
...
<script>
            ...
            $(document).ready(function () {
       $('#FinancingTypeTable_List').DataTable({
            // dom: 'Blfrtip',
            // dom: 'frtip',
...
<script>
    ...
    $("#AddFinancingTypeMaster_From").submit(function (e) {
        e.preventDefault();
        ...
        var redirect = "{{ route('admin.Green-Drive-Ev.master_management.financing_type_master.list') }}";
        ...
        $.ajax({
            url: "{{ route('admin.Green-Drive-Ev.master_management.financing_type_master.store') }}",
            type: "POST",
            data: formData,
            contentType: false,
            processData: false,
            success: function (response) {
                if (response.success) {
                    ...
                    $("#AddFinancingTypeMaster").modal('hide');
                    form.reset();
                    setTimeout(function () {
      ...
      $(document).on('click', '.editTelemetricOEMBtn', function () {
        var id = $(this).data('id');

        $.ajax({
            url: "{{ route('admin.Green-Drive-Ev.master_management.financing_type_master.get_data', ':id') }}".replace(':id', id),
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}"
            },
            success: function (response) {
                if (response.success) {
                    ...
                    $('#financing_type_name').val(response.data.name);
                     $('#edit_status').val(String(response.data.status)).trigger('change'); // 🔥 fixed here

                    $('#EditFinancingTypeMaster').modal('show');
                } else {
        ...
        $("#EditFinancingTypeMaster_From").submit(function (e) {
        e.preventDefault();
        ...
        $.ajax({
            url: "{{ route('admin.Green-Drive-Ev.master_management.financing_type_master.update') }}", // Update route
            type: "POST",
            data: formData,
            contentType: false,
            processData: false,
            success: function (response) {
                ...
                if (response.success) {
                    toastr.success(response.message);
                    $('#EditFinancingTypeMaster').modal('hide');
                    form.reset();
                    setTimeout(function () {
    ...
    $('.toggle-status').change(function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                // Proceed with AJAX
                $.ajax({
                    url: "{{ route('admin.Green-Drive-Ev.master_management.financing_type_master.status_update') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        id: Id,
...
<script>
  document.getElementById('exportBtn').addEventListener('click', function () {
    ...
    if (selected.length > 0) {
     ...
     const url = `{{ route('admin.Green-Drive-Ev.master_management.financing_type_master.export_financing_type_master') }}?${params.toString()}`;
