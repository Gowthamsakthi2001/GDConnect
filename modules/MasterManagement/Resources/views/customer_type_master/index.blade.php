    ...
    <div class="main-content">
                ...
                <div class="card-header" style="background:#fbfbfb;">
                    <div class="row g-3">
                        <div class="col-md-6 d-flex align-items-center">
                            <div class="card-title h5 custom-dark m-0">Customer Type Master <span class="badge text-muted shadow ms-2 p-2" style="background:rgb(234,234,234);">{{$data->count()}}</span></div>
                        ...
                        <div class="col-md-6 d-flex gap-2 align-items-center justify-content-end">
                            ...
                            <div class="text-center d-flex gap-2">
                                ...
                                <div class="m-2 btn btn-success d-flex align-items-center px-3"
                                    style="cursor: pointer;"      data-bs-toggle="modal" data-bs-target="#AddCustomerTypeMaster">
                    ...
                    <table id="ColorMasterTable_List" class="table text-left" style="width: 100%;">
                          <thead class="bg-white rounded" style="background:white !important; color:black !important;">
                            <tr>
                              ...
                              <th scope="col" class="custom-dark">Customer Type Name</th>
                        ...
                        <tbody class="bg-white border border-white">
                          ...
                          @if(isset($data))
                            @foreach($data as $val)
                                ...
                                {{-- Action --}}
                                <td class="text-start">
                                    <a href="javascript:void(0);" class="text-success editTypeBtn" data-id="{{ $val->id }}" data-name="{{$val->name}}" data-status="{{$val->status}}" style="font-size: 1.2rem;">
        ...
        <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasRightHR01" aria-labelledby="offcanvasRightHR01Label">
          <div class="offcanvas-header">
            <h5 class="custom-dark" id="offcanvasRightHR01Label">Color Master</h5>
            <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
          </div>
          <div class="offcanvas-body">
        
            <div class="d-flex gap-2 mb-3">
                <button class="btn btn-outline-secondary w-50" onclick="clearColorFilter()">Clear All</button>
                <button class="btn btn-success w-50" onclick="applyCOlorFilter()">Apply</button>
            </div>
        ...
        <div class="modal fade" id="AddCustomerTypeMaster" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
            aria-labelledby="AddCustomerTypeMaster" aria-hidden="true">
            <form id="AddCustomerTypeMaster_From" action="javascript:void(0);" method="POST">
                @csrf
                        ...
                        <div class="modal-body">
                            <div class="row">
                                 ...
                                 <div class="col-12 mb-3">
                                    <label for="vehicle_type" class="mb-2 ms-1">Customer Type
                                    <span class="text-danger">*</span></label>
                                        
                                    <input class="form-control basic-single" placeholder="Enter Customer Type" name="customer_type" required>
                                </div>
            ...
            <div class="modal fade" id="EditCustomerTypeMaster" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
            aria-labelledby="EditCustomerTypeMaster" aria-hidden="true">
            <form id="EditCustomerTypeMaster_From" action="javascript:void(0);" method="POST">
                @csrf
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" >Update Customer Type</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="row">
                                 ...
                                 <div class="col-12 mb-3">
                                    ...
                                    <input class="form-control basic-single" placeholder="Enter customer_type" name="customer_type_name" id="customer_type_name" required>
                                </div>
...
<script>
    $("#AddCustomerTypeMaster_From").submit(function (e) {
        e.preventDefault();
        ...
        var redirect = "{{ route('admin.Green-Drive-Ev.master_management.customer_type_master.index') }}";
        ...
        $.ajax({
            url: "{{ route('admin.Green-Drive-Ev.master_management.customer_type_master.store') }}",
            type: "POST",
            data: formData,
            contentType: false,
            processData: false,
            success: function (response) {
                if (response.success) {
                    ...
                    $("#AddCustomerTypeMaster").modal('hide');
                    form.reset();
                    setTimeout(function () {
    ...
    $(document).on('click', '.editTypeBtn', function () {
        ...
        $('#edit_status').val(String(status)).trigger('change');
    
        $('#EditCustomerTypeMaster').modal('show');
    });

    
        $("#EditCustomerTypeMaster_From").submit(function (e) {
        e.preventDefault();
        ...
        $.ajax({
            url: "{{ route('admin.Green-Drive-Ev.master_management.customer_type_master.update') }}", // Update route
            type: "POST",
            data: formData,
            contentType: false,
            processData: false,
            success: function (response) {
                ...
                if (response.success) {
                    toastr.success(response.message);
                    $('#EditCustomerTypeMaster').modal('hide');
                    form.reset();
                    setTimeout(function () {
    ...
    $('.toggle-status').change(function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: "{{ route('admin.Green-Drive-Ev.master_management.customer_type_master.status_update') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        id: Id,
...
<script>
  document.getElementById('exportBtn').addEventListener('click', function () {
    ...
    const url = `{{ route('admin.Green-Drive-Ev.master_management.customer_type_master.export') }}?${params.toString()}`;
