...
use Carbon\Carbon;
class ApprovedLeaveRequestDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('days', function ($row) {
                $no_of_type = $row->leave->days.' '.$row->leave->leave_type ;
                return $no_of_type ?? 'N/A';
            })
            ->addColumn('start_date', function ($row) {
             ...
             ->addColumn('permission_date', function ($row) {
                return $row->permission_date ? Carbon::parse($row->permission_date)->format('d-m-Y') : 'N/A';

            })
            ->addColumn('apply_days', function ($row) {
                if($row->leave->leave_type == 'day'){
                    $leave_type = $row->apply_days.' '.$row->leave->leave_type;
                    return $leave_type ?? 'N/A';
                }else{
                     $leave_type = $row->permission_hr.' '.$row->leave->leave_type;
                    return $leave_type ?? 'N/A';
                }
          ...
          ->addColumn('status', function ($row) {
                ...
                return $row->statusHandler();
            })

               ->addColumn('rejection_reason', function ($row) {
                return $row->rejection_reason ? $row->rejection_reason : 'N/A';
            })
    ...
    {
        return LeaveRequest::query()->with(['deliveryman','leave'])->whereNUll('req_status')->orderBy('id', 'desc');
    }
    
    public function html(): HtmlBuilder
    {
        return $this->builder()
            ->setTableId('approved-leave-request-table')
  ...
  public function getColumns(): array
    {
        return [
            ...
            Column::make('days')->title('No. Of Day/Hour')->defaultContent('N/A')->searchable(true),
            ...
            Column::make('permission_date')->title('Permission Date')->defaultContent('N/A')->searchable(true),
            Column::make('apply_days')->title('Applied')->defaultContent('N/A')->searchable(true),
            Column::make('status')->title('Status')->defaultContent('N/A')->searchable(true),
            Column::make('rejection_reason')->title('Rejected Reason')->defaultContent('N/A')->searchable(true),
